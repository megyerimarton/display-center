...
router.get('/orders', auth, async (req, res) => {
  const orders = await pool.query(`SELECT gyarto.name AS "gyarto", product.termek, product.kep1, order_item.ar, order_item.darab, order_item.ar, orders.allapot, orders.datum
...
router.get('/wishlist', auth, async (req, res) => {
  const wishlist = await pool.query(`SELECT gyarto.name AS "gyarto", product.kep1, product.id, product.termek, product.ar, product.akcios_ar, product.allapot
...
router.get('/userdata', auth, async (req, res) => {
  const result = await pool.query(`SELECT user.nev, user.email, cim.varos, cim.irszam, user.cim_id, user.utcaHazszam, user.tel
