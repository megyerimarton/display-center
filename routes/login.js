...
router.post('/', async (req, res) => {
  ...
  if (user.length > 0) {
    const token = jwt.sign({ id: user[0].id, username: user[0].email, isAdmin: false }, config.get('jwtPrivateKey'));
  ...
  if (admin.length > 0) {
    const token = jwt.sign({ id: admin[0].id, username: admin[0].username, isAdmin: true }, config.get('jwtPrivateKey'));
    return res.cookie('token', token).redirect(`/?message=success&text=Sikeres bejelentkezés`);
  }
