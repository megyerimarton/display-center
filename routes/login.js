...
router.post('/', async (req, res) => {
  const user = await pool.query(`SELECT id, email, pwd FROM user WHERE email = "${req.body.email}" AND pwd = "${md5(req.body.password)}" AND aktiv = 1`);
  ...
  const admin = await pool.query(`SELECT id, username, pwd FROM admin WHERE username = "${req.body.email}" AND pwd = "${md5(req.body.password)}"`);
