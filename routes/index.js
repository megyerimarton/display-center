...
router.get('/', async (req, res) => {
  ...
  const firstNew = (await pool.query('SELECT id, cim, DATE(datum) AS "datum", tartalom, kep FROM news WHERE aktiv = 1 AND id = (SELECT MAX(id) FROM news)'))[0];
  const news = await pool.query('SELECT id, cim, DATE(datum) AS "datum", tartalom, kep FROM news WHERE aktiv = 1 ORDER BY id DESC LIMIT 3 OFFSET 1');
